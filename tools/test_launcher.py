...
class LauncherTests(unittest.TestCase):
    def test_non_tty_ports_launch_bootstraps_versioned_app_and_uses_vaixterm(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (ports/'cozos/updater.py').write_text('VERSION="0.6.3"\n')
            ...
            self.assertEqual((state/'active-version').read_text().strip(),'0.6.3')
            entry=state/'apps/0.6.3/main.py'
            self.assertTrue(entry.is_file())
            arguments=log.read_text().splitlines()
            expected='cd "'+str(state/'apps/0.6.3')+'" && python3 "'+str(entry)+'"'
