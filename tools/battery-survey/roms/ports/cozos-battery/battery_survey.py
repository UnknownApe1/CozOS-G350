#!/usr/bin/env python3
"""Read-only BATLEXP G350/RK817 battery survey logger."""
import csv
import datetime
import json
import os
...
VERSION = '0.1'
ROOT = Path(os.environ.get('COZOS_TEST_ROOT', '/'))
...
def start():
    ...
    try:
        ...
        error_log.close()
    for _ in range(20):
        if PID_FILE.exists() and alive(proc.pid):
...
def package():
    ...
    session = Path(session_name) if session_name else None
